...
import payment.api.tx.cmb.Tx7281Request;
...
public class Tx7281 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            // 2.创建交易请求对象
            Tx7281Request tx7281Request = new Tx7281Request();
            tx7281Request.setInstitutionID(institutionID);
            tx7281Request.setBatchNo(batchNo);
            tx7281Request.setTotalCount(totalCount);
            tx7281Request.setTotalAmount(totalAmount);
            tx7281Request.setBatchRemark(batchRemark);
            ...
            if (txSN != null && txSN.length > 0) {
                ...
                for (int i = 0; i < txSN.length; i++) {
                    ...
                    for (int j = 0; j < count; j++) {
                        ...
                        if ("30".equals(settlementType[i]) || "32".equals(settlementType[i])) {
            ...
            tx7281Request.setItemList(itemList);

            // 3.执行报文处理
            tx7281Request.process();

            // 4.将参数放置到request对象
            request.setAttribute("plainText", tx7281Request.getRequestPlainText());
            request.setAttribute("message", tx7281Request.getRequestMessage());
            request.setAttribute("signature", tx7281Request.getRequestSignature());
            request.setAttribute("txCode", "7281");
