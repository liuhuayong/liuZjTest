...
import payment.api.tx.depositbank.Tx4722Request;
import payment.api.vo.DepositItem;


public class Tx4722 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            // 2.创建交易请求对象 
            Tx4722Request tx4722Request = new Tx4722Request();
            tx4722Request.setInstitutionID(institutionID);
            ...
            tx4722Request.setItemList(itemList);
            // 3.执行报文处理
            tx4722Request.process();
            
            // 4.将参数放置到request对象
            // //3个交易参数
            request.setAttribute("plainText", tx4722Request.getRequestPlainText());
            request.setAttribute("message", tx4722Request.getRequestMessage());
            request.setAttribute("signature", tx4722Request.getRequestSignature());
            // //2个信息参数
            request.setAttribute("txCode", "4722");
            request.setAttribute("txName", "电子账户密码设置批量查询");
            // 1个action(支付平台地址)参数
