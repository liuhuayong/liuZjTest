...
import payment.api.tx.depositbank.Tx4732Request;
import payment.api.vo.DepositItem;




public class Tx4732 extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            // 2.创建交易请求对象
            Tx4732Request tx4732Request = new Tx4732Request();
            tx4732Request.setInstitutionID(institutionID);
            ...
            tx4732Request.setItemList(itemList);
            // 3.执行报文处理
            tx4732Request.process();

            // 4.将参数放置到request对象
            request.setAttribute("plainText", tx4732Request.getRequestPlainText());
            request.setAttribute("message", tx4732Request.getRequestMessage());
            request.setAttribute("signature", tx4732Request.getRequestSignature());
            request.setAttribute("txCode", "4732");
            request.setAttribute("txName", "银行卡绑定状态批量查询");
