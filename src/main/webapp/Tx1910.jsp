...
function CheckAll(){
  ...
  //校验付款方账户类型
  var gatheringAccountType= document.getElementsByName("GatheringAccountType");
  for (var i=0;i<gatheringAccountType.length;i++){
    if (gatheringAccountType[i].value==null||gatheringAccountType[i].value==""){
      alert("第"+(i+1)+"个明细记录的付款方账户类型不能为空");
      return false;
    }
  }
  
  //校验收款方账户类型
  var paymentAccountType= document.getElementsByName("PaymentAccountType");
  for (var i=0;i<paymentAccountType.length;i++){
    if (paymentAccountType[i].value==null||paymentAccountType[i].value==""){
      alert("第"+(i+1)+"个明细记录的收款方账户类型不能为空");
      return false;
    }
  }
...
window.onload = function(){
   ...
   addItem();
    ...
    <%
        String action = contextPath + "/Tx1910";
    %>
    <form action="<%=action%>" name="form1" method="POST">
      <table width="100%" cellpadding="2" cellspacing="1" border="0" align="center" bgcolor="#DDDDDD">
        ...
        <tr class="mouseout">
          ...
          <td width="*" class="content">
            <input type="text" name="BatchNo" size="40" maxlength="32" value="<%=GUIDGenerator.genGUID()%>" />
          </td>
        </tr>
        <tr class="mouseout">
        ...
        <tr class="mouseout">
          ...
          <!--   <input type="text" name="Remark" size="40" value="" /> -->
            <textarea rows="3" cols="" name="Remark" style="width: 275px"></textarea>
          </td>
        </tr>
        <tr class="mouseout">
        <td width="120" class="label" height="24">明细</td>
        <td width="*" class="content" id="Item">
        <a href="javascript:void(0)" style="font-weight:bold" onclick="addItem()">添加明细</a>
        </td>
        </tr>
      </table>
        ...
        <tr>
          ...
          <td width="*">
            <input type="button" class="ButtonMouseOut" onmouseover="this.className='ButtonMouseOver'"
              onmouseout="this.className='ButtonMouseOut'" style="width: 60px;" value="提交" style="font-weight:bold" onclick="doSubmit()">
          </td>
        </tr>
      </table>
